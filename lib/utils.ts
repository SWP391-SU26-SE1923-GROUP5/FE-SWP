...
export const parseAsUTC = (dateInput: string | Date | undefined | null): Date => {
  if (!dateInput) return new Date();
  if (typeof dateInput === 'string') {
    if (!dateInput.endsWith('Z') && !dateInput.match(/[+-]\d{2}:\d{2}$/)) {
...
export const formatDateGMT7 = (
  ...
  if (options && ('timeStyle' in options || 'hour' in options || 'minute' in options)) {
    return date.toLocaleString('vi-VN', {
      ...options,
      timeZone: 'Asia/Ho_Chi_Minh'
    });
