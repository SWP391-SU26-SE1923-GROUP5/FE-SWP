...
export const updateEditedFile = async ({ fileId, oldBucketFileId, file, path }: { fileId: string; oldBucketFileId: string; file: File; path: string }) => {
    try {
        ...
        const newBucketFile = await storage.createFile(
            appwriteConfig.bucketId,
            ID.unique(),
            inputFile
        );
        ...
        handleError(error, "Failed to update edited file");
    }
}
...
export const getFileBuffer = async (bucketFileId: string): Promise<Buffer> => {
    if (!bucketFileId) throw new Error("Missing file id.");
    try {
        const { storage } = await createAdminClient();
        const result = await storage.getFileDownload(appwriteConfig.bucketId, bucketFileId);
        return Buffer.from(result);
    } catch (error) {
        handleError(error, "Failed to fetch file buffer.");
        throw error;
    }
};